...
A worker whose job is "read this 500-word article and extract the main claim" handles that well on a fast, inexpensive model. Every provider has a tier for this: Anthropic's Haiku 4.5, OpenAI's GPT-5.4 mini, Google's Gemini 3.1 Flash Lite. For even simpler tasks like formatting, extraction, or classification where reasoning depth doesn't matter, OpenAI's GPT-5.4 Nano pushes costs lower still. The capable model is only needed where it earns its cost.
